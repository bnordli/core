...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    for identity, entity_info in plejdinfo["config"].get(CONF_BINARY_SENSORS).items():
        i = int(identity)
        _LOGGER.debug(f"Adding button {i} ({entity_info[CONF_NAME]})")
        button = PlejdButton(entity_info[CONF_NAME], i, service)
